...
<h1>CERTIFICATIONS</h1>
  ...
  <defs>
    {/* Fill gradient for inner circle */}
   

    <linearGradient id={`gradient${cert.id}`} x1="0%" y1="0%" x2="100%" y2="100%"> <stop offset="0%" style={{ stopColor: "#57370d", stopOpacity: 1 }} /> <stop offset="100%" style={{ stopColor: "#ffe998", stopOpacity: 1 }} /> </linearGradient>



    {/* Stroke gradient */}
    <linearGradient
      id={`strokeGradient${cert.id}`}
      x1="0%"
      y1="0%"
      x2="0%"
      y2="100%"
    >
      <stop offset="0%" stopColor="#6b2cf5" />
      <stop offset="50%" stopColor="#d450e6" />
      <stop offset="100%" stopColor="#6db3fc" />
    </linearGradient>
  ...
  fill={`url(#gradient${cert.id})`}
  transform="translate(14,14) scale(0.5) translate(-12,-12)" 
/> */}

  <circle
    cx="14"
    cy="14"
    r="10"                 // radius of inner circle
    fill="transparent"         // or use your gradient: fill={`url(#gradient${cert.id})`}
    stroke={`url(#strokeGradient${cert.id})`} // optional gradient stroke
    strokeWidth="0.4"
  />

  {/* Circular text on top */}
  <text fill="black" fontSize="2">
    <textPath href={`#circlePath${cert.id}`} startOffset="0%" method="stretch" spacing="auto">
  ...
  <text fill="black" fontSize="2">
    <textPath href={`#circlePath${cert.id}`} startOffset="50%" method="stretch" spacing="auto">
      CERTIFIED • {cert.name} • COMPLETED
            ...
            <p><strong>Expected Completion</strong>: {cert.completion}</p>
            <a className="certificate-link" href={cert.certificateLink}>Certificate</a> */}
          </div>
        ))}
      </div>
      </div>
